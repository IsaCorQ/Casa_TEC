...
function App() {
  ...
  const toggleLuz = async (luz) => {
    try {
      ...
      obtenerEstadoLuces(); // Actualizar el estado de las luces
    } catch (error) {
      console.error("Error cambiando el estado de la luz:", error);
    }
  };

  // Función para obtener el estado de las puertas desde el servidor
  const obtenerEstadoPuertas = async () => {
    try {
      ...
      console.error("Error obteniendo estado de las puertas:", error);
    }
  };
